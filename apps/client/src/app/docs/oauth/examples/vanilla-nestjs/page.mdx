...
const CONFIG = {
    ...
    AUTH_URL: 'https://oauth.datagsm.com/login',
};
...
src/
├── auth/
│   ├── auth.controller.ts       # OAuth 인증 컨트롤러
│   ├── auth.service.ts          # OAuth 비즈니스 로직
│   ├── auth.module.ts           # 인증 모듈
│   ├── dto/
│   │   ├── callback.dto.ts      # 콜백 요청 DTO
│   │   └── user-info.dto.ts     # 사용자 정보 DTO
│   └── guards/
│       └── session.guard.ts     # 세션 인증 가드
├── user/
...
export class AuthService {
  ...
  private async fetchUserInfo(accessToken: string): Promise<DataGsmUserInfo> {
    const userInfoUrl = this.configService.get<string>(
      ...
      'https://oauth.datagsm.com/userinfo',
    );
...
DATAGSM_TOKEN_URL=https://oauth.datagsm.com/v1/oauth/token
DATAGSM_USERINFO_URL=https://oauth.datagsm.com/userinfo
