  ...
  <div className="flex items-start gap-3">
    ...
    <div>
      ...
      <div className="text-red-800 dark:text-red-200 text-sm">
        ...
        서버에서 <code className="mx-1 px-1.5 py-0.5 bg-red-100 dark:bg-red-800 rounded">client_secret</code>을 사용하는 것은 가능하지만,
        새로운 프로젝트에서는 <strong>PKCE</strong>를 사용하는 것을 권장합니다.
...
const CONFIG = {
    ...
    AUTH_URL: 'https://oauth.data.hellogsm.kr/login',
};
...
export class AuthService {
  ...
  private async fetchUserInfo(accessToken: string): Promise<DataGsmUserInfo> {
    const userInfoUrl = this.configService.get<string>(
      ...
      'https://oauth.data.hellogsm.kr/userinfo',
    );
...
DATAGSM_USERINFO_URL=https://oauth.data.hellogsm.kr/userinfo
