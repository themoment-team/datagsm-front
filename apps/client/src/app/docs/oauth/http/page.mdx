...
인증 코드 발급 엔드포인트는 다음과 같은 응답을 반환합니다.
...
| 엔드포인트 | 메서드 | 설명 | 문서 |
| --- | --- | --- | --- |
| `/v1/oauth/code` | `POST` | 사용자 인증 후 Authorization Code 발급 | [상세 문서](/docs/oauth/http/code) |
| `/v1/oauth/token` | `POST` | Authorization Code를 Access Token으로 교환 | [상세 문서](/docs/oauth/http/token) |
| `/v1/oauth/refresh` | `PUT` | Refresh Token으로 새 Access Token 발급 | [상세 문서](/docs/oauth/http/refresh) |
...
| `401 Unauthorized` | 인증 실패 (잘못된 비밀번호, 만료된 토큰 등) | 사용자 인증 정보나 토큰이 유효한지 확인하세요. |
| `403 Forbidden` | 권한 없음 (잘못된 Client Secret 등) | Client Secret이 올바른지, 리다이렉트 URI가 등록되었는지 확인하세요. |
| `404 Not Found` | 리소스를 찾을 수 없음 (존재하지 않는 Code나 Token) | Code나 Token이 만료되었거나 이미 사용된 것은 아닌지 확인하세요. |
| `429 Too Many Requests` | 요청량 제한 초과 | 잠시 후 다시 시도하거나 요청 빈도를 줄이세요. |
...
```bash
curl -X GET "https://oauth.datagsm.com/userinfo" \
...
- 과도한 요청은 `429 Too Many Requests` 오류를 발생시킵니다.
- 적절한 재시도 로직을 구현하세요 (Exponential Backoff 권장).

### 다음 단계

각 엔드포인트의 상세한 사용법은 다음 문서를 참고하세요.

- [인증 코드 발급](/docs/oauth/http/code) - 사용자 인증 및 Authorization Code 획득
