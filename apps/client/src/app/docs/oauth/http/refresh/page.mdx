...
```bash
curl -X PUT "https://oauth.datagsm.com/v1/oauth/refresh" \
...
<CodeTabs>
  ...
  try {
    const response = await fetch('https://oauth.datagsm.com/v1/oauth/refresh', {
      method: 'PUT',
      headers: {
...
def refresh_access_token(refresh_token):
    ...
    try:
        response = requests.put(
            'https://oauth.datagsm.com/v1/oauth/refresh',
...
public class OAuthRefreshService {
    ...
    public static TokenResponse refreshAccessToken(String refreshToken) throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create("https://oauth.datagsm.com/v1/oauth/refresh"))
