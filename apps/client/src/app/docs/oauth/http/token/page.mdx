...
Authorization Code를 Access Token과 Refresh Token으로 교환합니다. 보안을 위해 **PKCE (Proof Key for Code Exchange) 방식**을 사용하여 `code_verifier`로 인증하는 것을 강력히 권장합니다.
...
<div className="border-l-4 border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded my-6">
  <div className="flex items-start gap-3">
    <Shield className="h-5 w-5 text-yellow-600 dark:text-yellow-500 shrink-0 mt-0.5" />
    <div>
      <div className="font-semibold text-yellow-900 dark:text-yellow-100 mb-1">
        PKCE 방식 사용 권장
      </div>
      <div className="text-yellow-800 dark:text-yellow-200 text-sm">
        Authorization Code Grant에서는 <code className="mx-1 px-1.5 py-0.5 bg-yellow-100 dark:bg-yellow-800 rounded">code_verifier</code>를 사용하는 PKCE 방식을 권장합니다.
        <code className="mx-1 px-1.5 py-0.5 bg-yellow-100 dark:bg-yellow-800 rounded">client_secret</code> 방식은 레거시 지원용으로, 보안상 새로운 구현에서는 사용하지 않는 것을 권장합니다.
      </div>
    </div>
  </div>
</div>
  ...
  <div className="flex items-start gap-3">
    ...
    <div>
      ...
      <div className="text-red-800 dark:text-red-200 text-sm">
        <code className="mx-1 px-1.5 py-0.5 bg-red-100 dark:bg-red-800 rounded">client_secret</code>는 레거시 지원 목적으로만 제공됩니다.
        클라이언트에서 secret을 안전하게 보관하기 어려우므로, PKCE 방식(<code className="mx-1 px-1.5 py-0.5 bg-red-100 dark:bg-red-800 rounded">code_verifier</code>)을 사용해주세요.
      </div>
    </div>
  </div>
</div>
...
**주의**: `client_secret` 방식은 클라이언트 사이드에서 안전하게 관리하기 어려우므로, 가능한 빠른 시일 내에 PKCE 방식으로 마이그레이션하는 것을 강력히 권장합니다.
