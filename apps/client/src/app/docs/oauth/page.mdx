...
REST API를 직접 호출하여 구현합니다. 가장 유연하지만 구현 복잡도가 높습니다. [HTTP API 문서](/docs/oauth/http), [인증 코드 발급](/docs/oauth/http/code), [토큰 교환](/docs/oauth/http/token-exchange), [토큰 갱신](/docs/oauth/http/token-refresh), [사용자 정보 조회](/docs/oauth/http/userinfo)
