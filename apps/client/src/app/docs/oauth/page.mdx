  ...
  <div className="flex items-start gap-3">
    ...
    <div>
      ...
      <div className="text-yellow-800 dark:text-yellow-200 text-sm">
        보안 강화를 위해 PKCE (Proof Key for Code Exchange) 사용을 권장합니다.
        <code className="mx-1 px-1.5 py-0.5 bg-yellow-100 dark:bg-yellow-800 rounded">client_secret</code> 방식은 레거시 지원 목적으로만 제공되며, 새로운 애플리케이션에서는 PKCE를 사용해주세요.
...
- **Client Secret**: 애플리케이션의 비밀 키 (**레거시 방식**, PKCE 사용 권장)
...
PKCE는 Authorization Code 탈취 공격을 방지하는 가장 효과적인 방법입니다. 다음 사항을 준수하세요:
  ...
  <div className="flex items-start gap-3">
    ...
    <div>
      ...
      <div className="text-red-800 dark:text-red-200 text-sm">
        <code className="mx-1 px-1.5 py-0.5 bg-red-100 dark:bg-red-800 rounded">client_secret</code>을 사용하는 방식은 레거시 지원 목적으로만 제공됩니다.
        클라이언트 사이드에서 secret을 안전하게 보관하기 어려우므로, <strong>새로운 애플리케이션에서는 PKCE를 사용해주세요</strong>.
      </div>
    </div>
  </div>
</div>
