...
| `abolishedYear` | `Int?`                     | 폐지연도 (운영 중인 경우 `null`)                   | `2024`       |
...
```bash
curl -X GET "https://openapi.datagsm.kr/v1/clubs?clubType=MAJOR_CLUB&page=0&size=50" \
...
```bash
# 동아리 이름 순으로 정렬
curl -X GET "https://openapi.datagsm.kr/v1/clubs?sortBy=NAME&sortDirection=ASC&page=0&size=50" \
  -H "X-API-KEY: your-api-key-here"
```
...
동아리 부장이 자퇴 또는 졸업 처리된 경우 `leader` 필드는 `null`로 반환됩니다.
...
async function getClubs(clubId) {
  try {
    const response = await axios.get('https://openapi.datagsm.kr/v1/clubs', {
      headers: {
        'X-API-KEY': 'your-api-key-here',
      },
      params: {
...
def get_club(club_id):
    try:
        response = requests.get(
            'https://openapi.datagsm.kr/v1/clubs',
...
public class ClubAPI {
    public static String getClubs(Long clubId) throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create("https://openapi.datagsm.kr/v1/clubs?clubId=" + clubId))
