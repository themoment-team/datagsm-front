...
- `fromDate`와 `toDate`: 기간 범위 내의 급식 데이터 조회
...
| `data`    | `Object` | 급식 응답 객체    | -     |

### data 객체

| 필드      | 타입      | 설명           | 예시 |
|---------|---------|--------------|----|
| `meals` | `Array` | 급식 데이터 목록 배열 | -  |

### meals 배열

`meals` 필드는 급식 데이터 객체의 배열이며, 각 급식 객체는 다음 필드를 포함합니다.
...
```bash
# 특정 날짜 조회
curl -X GET "https://openapi.datagsm.kr/v1/neis/meals?date=2025-12-15" \
  -H "X-API-KEY: your-api-key-here"

# 기간 범위 조회
curl -X GET "https://openapi.datagsm.kr/v1/neis/meals?fromDate=2025-12-01&toDate=2025-12-31" \
  -H "X-API-KEY: your-api-key-here"
```
...
async function getMeals(date) {
  try {
    const response = await axios.get('https://openapi.datagsm.kr/v1/neis/meals', {
      headers: {
...
def get_meals(date):
    try:
        response = requests.get(
            'https://openapi.datagsm.kr/v1/neis/meals',
...
public class NeisAPI {
    public static String getMeals(String date) throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create("https://openapi.datagsm.kr/v1/neis/meals?date=" + date))
