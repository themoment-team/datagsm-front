...
| 엔드포인트                     | 설명               | 필요 권한 범위       |
|---------------------------|------------------|----------------|
| `GET /v1/students`        | 학생 데이터 조회        | `STUDENT_READ` |
| `GET /v1/clubs`           | 동아리 데이터 조회       | `CLUB_READ`    |
| `GET /v1/projects`        | 프로젝트 데이터 조회      | `PROJECT_READ` |
| `GET /v1/neis/meals`      | NEIS 급식 데이터 조회   | `NEIS_READ`    |
| `GET /v1/neis/schedules`  | NEIS 학사일정 데이터 조회 | `NEIS_READ`    |
| `GET /v1/neis/timetables` | NEIS 시간표 데이터 조회  | `NEIS_READ`    |
| `POST /v1/webhooks`       | Webhook 등록       | `WEBHOOK_WRITE` |
| `GET /v1/webhooks`        | Webhook 목록 조회    | `WEBHOOK_WRITE` |
| `PATCH /v1/webhooks/{webhookId}` | Webhook 수정 | `WEBHOOK_WRITE` |
| `DELETE /v1/webhooks/{webhookId}` | Webhook 삭제 | `WEBHOOK_WRITE` |
...
```bash
curl -X GET "https://openapi.datagsm.kr/v1/students?grade=1&classNum=1" \
...
- [학생 데이터 OpenAPI](/api/http/student)
- [동아리 데이터 OpenAPI](/api/http/club)
- [프로젝트 데이터 OpenAPI](/api/http/project)
- [NEIS 데이터 OpenAPI](/api/http/neis)

이벤트 기반 통합이 필요하다면 [Webhook 기술 문서](/api/webhook)를 참고하세요. SDK를 사용한 더 편리한 API 호출을 원한다면 [SDK 기술 문서](/api/sdk)를 참고하세요.
