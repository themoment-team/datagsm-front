...
| `status`        | `ProjectStatus` | 선택    | 프로젝트 운영 상태 (`ACTIVE`, `ENDED`, 기본값: `ACTIVE`)        | `ACTIVE`       |
...
| `startYear`    | `Int`                      | 프로젝트 시작 연도                                   | `2024`                 |
| `endYear`      | `Int?`                     | 프로젝트 종료 연도 (운영 중인 경우 `null`)                 | `2025`                 |
| `status`       | `ProjectStatus`            | 프로젝트 운영 상태 (`ACTIVE`: 운영 중, `ENDED`: 종료)    | `ACTIVE`               |
...
```bash
curl -X GET "https://openapi.datagsm.kr/v1/projects?clubId=1&page=0&size=50" \
...
```bash
# 프로젝트 이름 순으로 정렬
curl -X GET "https://openapi.datagsm.kr/v1/projects?sortBy=NAME&sortDirection=ASC&page=0&size=50" \
  -H "X-API-KEY: your-api-key-here"
```
...
async function getProjects(projectId) {
  try {
    const response = await axios.get('https://openapi.datagsm.kr/v1/projects', {
      headers: {
        'X-API-KEY': 'your-api-key-here',
      },
      params: {
...
def get_project(project_id):
    try:
        response = requests.get(
            'https://openapi.datagsm.kr/v1/projects',
...
public class ProjectAPI {
    public static String getProjects(Long projectId) throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create("https://openapi.datagsm.kr/v1/projects?projectId=" + projectId))
