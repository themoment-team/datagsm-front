...
| `specialty`        | `String`        | 선택    | 전공                                                                                                  | `백엔드`               |
| `major`            | `Major`         | 선택    | 학과 (`SW_DEVELOPMENT`, `SMART_IOT`, `AI`)                                                            | `SW_DEVELOPMENT`    |
| `githubId`         | `String`        | 선택    | GitHub 아이디                                                                                          | `octocat`           |
...
| `major`          | `Major?`          | 학과 (졸업/자퇴 시 `null`, `SW_DEVELOPMENT`, `SMART_IOT`, `AI`)                                   | `SW_DEVELOPMENT`             |
...
| `githubId`       | `String?`         | GitHub 아이디 (미설정 시 `null`)                                                                  | `octocat`                    |
| `githubUrl`      | `String?`         | GitHub 프로필 URL (미설정 시 `null`)                                                              | `https://github.com/octocat` |
...
```bash
curl -X GET "https://openapi.datagsm.kr/v1/students?grade=1&classNum=1&page=0&size=50" \
...
```bash
# 학번 순으로 정렬
curl -X GET "https://openapi.datagsm.kr/v1/students?sortBy=STUDENT_NUMBER&sortDirection=ASC&page=0&size=50" \
  -H "X-API-KEY: your-api-key-here"

# 이름 역순으로 정렬
curl -X GET "https://openapi.datagsm.kr/v1/students?sortBy=NAME&sortDirection=DESC&page=0&size=50" \
  -H "X-API-KEY: your-api-key-here"
```
...
async function getStudents(studentId) {
  try {
    const response = await axios.get('https://openapi.datagsm.kr/v1/students', {
      headers: {
        'X-API-KEY': 'your-api-key-here',
      },
      params: {
...
def get_student(student_id):
    try:
        response = requests.get(
            'https://openapi.datagsm.kr/v1/students',
...
public class StudentAPI {
    public static String getStudents(Long studentId) throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create("https://openapi.datagsm.kr/v1/students?studentId=" + studentId))
