...
**DataGSM OpenAPI SDK Java**는 DataGSM API를 Java 및 Kotlin 애플리케이션에서 쉽고 안전하게 사용할 수 있도록 설계된 공식 클라이언트 SDK입니다.
...
| **Optional API 지원** | nullable 필드를 `Optional<T>`로 안전하게 처리하여 `NullPointerException`을 방지합니다. |
...
<CodeTabs>
  <CodeTab label="Java" language="java" code={`import team.themoment.datagsm.sdk.openapi.DataGsmOpenApiClient;
...
DataGsmOpenApiClient clientWithCustomUrl = DataGsmOpenApiClient.builder("your-api-key-here")
  ...
  <CodeTab label="Kotlin" language="kotlin" code={`import team.themoment.datagsm.sdk.openapi.DataGsmOpenApiClient
...
#### 재학 상태 필터링 (`EnrollmentFilter`)

`StudentRequest`의 재학 상태 필터링은 `EnrollmentFilter` Type-State DSL을 통해 구성합니다. 우선순위를 위반하는 파라미터 조합(예: `onlyEnrolled=true` + `includeGraduates=true`)은 타입 시스템에 의해 **컴파일 타임에 방지**됩니다.
...
| _(필터 미설정)_                      | 서버 기본값 사용                                   |

<CodeTabs>
  <CodeTab label="Java" language="java" code={`import team.themoment.datagsm.sdk.openapi.model.EnrollmentFilter;
...
// 서버 기본값 사용 (파라미터 미전송)
StudentApi.StudentRequest defaultRequest = new StudentApi.StudentRequest();`} />

  <CodeTab label="Kotlin" language="kotlin" code={`import team.themoment.datagsm.sdk.openapi.model.EnrollmentFilter
...
// 서버 기본값 사용 (파라미터 미전송)
val defaultRequest = StudentApi.StudentRequest()`} />
...
<CodeTabs>
  <CodeTab label="Java" language="java" code={`// 급식 데이터 조회
List<Meal> meals = client.neis().getMeals(request);

// 학사일정 조회
List<Schedule> schedules = client.neis().getSchedules(request);`} />

  <CodeTab label="Kotlin" language="kotlin" code={`// 급식 데이터 조회
val meals = client.neis().getMeals(request)
...
### Nullable 필드 & Optional API

SDK PR #5부터 일부 getter의 반환 타입이 `Optional<T>`로 변경되었습니다. 이를 통해 `null` 체크를 명시적으로 강제하여 `NullPointerException`을 방지할 수 있습니다.

#### Nullable 필드 목록

| 클래스             | 필드 / 메서드                  | 반환 타입                        | 설명                          |
|------------------|-----------------------------|---------------------------------|-----------------------------|
| `ClubDetail`     | `getLeader()`               | `Optional<ParticipantInfo>`     | 부장이 없는 경우 비어있음             |
| `Student`        | `getStudentNumber()`        | `Optional<String>`              | 졸업/자퇴 시 비어있음               |
| `Student`        | `getGrade()`                | `Optional<Integer>`             | 졸업/자퇴 시 비어있음               |
| `Student`        | `getClassNum()`             | `Optional<Integer>`             | 졸업/자퇴 시 비어있음               |
| `Student`        | `getMajor()`                | `Optional<Major>`               | 졸업/자퇴 시 비어있음               |
| `Meal`           | `getMealType()`             | `Optional<MealType>`            | 급식 정보가 없는 경우 비어있음          |
| `Schedule`       | `getContent()`              | `Optional<String>`              | 일정 내용이 없는 경우 비어있음          |
| `Project`        | `getClub()`                 | `Optional<ClubSummary>`         | 동아리 연계가 없는 경우 비어있음         |

#### Optional 사용 패턴

<CodeTabs>
  <CodeTab label="Java" language="java" code={`// ClubDetail.getLeader() — Optional<ParticipantInfo>
ClubDetail club = client.clubs().getClub(1L);

// ifPresent: 값이 있을 때만 실행
club.getLeader().ifPresent(leader -> {
    System.out.println("부장: " + leader.getName());
});

// orElse: 기본값 지정
String leaderName = club.getLeader()
    .map(ParticipantInfo::getName)
    .orElse("부장 없음");

// Student nullable 필드 예시
Student student = client.students().getStudent(1L);
...
String major = student.getMajor()
    .map(Enum::name)
    .orElse("정보 없음");
String studentNumber = student.getStudentNumber().orElse("N/A");`} />

  <CodeTab label="Kotlin" language="kotlin" code={`// ClubDetail.getLeader() — Optional<ParticipantInfo>
val club = client.clubs().getClub(1L)

// ifPresent: 값이 있을 때만 실행
club.getLeader().ifPresent { leader ->
    println("부장: \${leader.name}")
}

// orElse: 기본값 지정
val leaderName = club.getLeader()
    .map { it.name }
    .orElse("부장 없음")
...
val grade = student.getGrade().orElse(null)
val major = student.getMajor().map { it.name }.orElse("정보 없음")
val studentNumber = student.getStudentNumber().orElse("N/A")`} />
...
Spring Boot 애플리케이션에서는 `DataGsmOpenApiClient`를 Bean으로 등록하여 의존성 주입 방식으로 사용할 수 있습니다.
...
fun main() {
    ...
    DataGsmOpenApiClient.builder("your-api-key-here").build().use { client ->
        ...
        students.students.forEach { student ->
            println("\${student.name} (\${student.getStudentNumber().orElse("N/A")}) - \${student.email}")
        }
