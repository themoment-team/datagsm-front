...
DataGsmOpenApiClient clientWithCustomUrl = DataGsmOpenApiClient.builder("your-api-key-here")
    .baseUrl("https://openapi.datagsm.kr") // 설정하지 않는다면 공식 Base URL 사용
    .build();`} />
...
| `getTimetables(request)`   | 시간표 데이터 조회 | `List<Timetable>`   |
...
// 시간표 조회
List<Timetable> timetables = client.neis().getTimetables(request);`} />
...
// 시간표 조회
val timetables = client.neis().getTimetables(request)`} />
...
datagsm:
  api:
    ...
    base-url: https://openapi.datagsm.kr
```
