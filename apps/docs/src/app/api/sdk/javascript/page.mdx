...
**DataGSM OpenAPI SDK for JavaScript**는 DataGSM API를 Node.js 및 브라우저 환경에서 쉽고 안전하게 사용할 수 있도록 설계된 공식 클라이언트 SDK입니다.
...
<CodeTabs>
  <CodeTab label="npm" language="bash" code={`npm install @themoment-team/datagsm-openapi`} />
  <CodeTab label="pnpm" language="bash" code={`pnpm add @themoment-team/datagsm-openapi`} />
  <CodeTab label="yarn" language="bash" code={`yarn add @themoment-team/datagsm-openapi`} />
</CodeTabs>
...
<CodeTabs>
  <CodeTab label="JavaScript" language="typescript" code={`
  // 학생 목록 조회 (1학년 1반)
