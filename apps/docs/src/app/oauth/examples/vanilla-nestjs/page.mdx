...
프론트엔드(Vanilla JavaScript)에서 Authorization Code까지 획득하고, 서버(NestJS)에서 Token 교환 및 사용자 데이터 조회 후 세션 기반 인증을 구현하는 OAuth 예제입니다.
...
| 장점 | 설명 |
| --- | --- |
| **보안성** | 세션 ID만 클라이언트에 전달되므로 사용자 데이터 노출 최소화 |
    ...
    <div class="callback-container">
        <div id="loadingContainer" class="loading-container">
            ...
            <div class="loading-text">
                <h2>로그인 처리 중</h2>
                <p id="progressText">인증 데이터를 확인하는 중입니다...</p>
            </div>
        </div>
...
const CONFIG = {
    ...
    AUTH_URL: 'https://oauth.authorization.datagsm.kr/v1/oauth/authorize',
};
...
async function exchangeCodeForSession(code) {
    updateProgress('서버에서 인증 데이터를 처리하는 중...');
...
async function handleOAuthCallback() {
    try {
        ...
        updateProgress('사용자 데이터를 가져오는 중...');
...
graph TD
    ...
    B4 --> B4B[user-info.dto.ts<br/><i>사용자 데이터 DTO</i>]
...
export class AuthController {
  ...
  async handleCallback(
    ...
    try {
      // Authorization Code로 사용자 데이터 조회 및 DB 동기화
...
- [NestJS 공식 기술 문서](https://docs.nestjs.com/)
- [Express Session 기술 문서](https://github.com/expressjs/session)
