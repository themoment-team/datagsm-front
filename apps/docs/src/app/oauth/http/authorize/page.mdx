...
| `scope`                 | `String` | 선택        | 요청할 OAuth Scope 목록 (공백으로 구분). 미지정 시 Client의 전체 권한 범위 사용 | `a1b2c3d4-e5f6-7890-abcd-ef1234567890:profile a1b2c3d4-e5f6-7890-abcd-ef1234567890:email` |
...
| 상태 코드               | 설명       | 원인                                                                           |
|---------------------|----------|------------------------------------------------------------------------------|
| `400 Bad Request`   | 잘못된 요청   | `response_type`이 `code`가 아님, 등록되지 않은 `redirect_uri`, `code_challenge` 없이 `code_challenge_method`만 전달 |
...
<div className="flex items-center gap-2 mt-4">
  ...
  <a href="/oauth/http/token-exchange" className="underline font-medium">토큰 교환</a>
