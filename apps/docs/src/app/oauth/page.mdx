...
| **Redis 캐싱**  | Redis를 활용하여 토큰 데이터를 캐싱하고 빠른 검증을 제공합니다.                                  |
...
| **서드파티 권한 범위** | DataGSM OAuth를 통해 외부 앱의 권한을 사용할 수 있습니다. |
...
// 인증 요청 시 포함
const authUrl = `https://oauth.authorization.datagsm.kr/v1/oauth/authorize?client_id=${clientId}&redirect_uri=${redirectUri}&state=${state}`;
...
PKCE를 사용한 안전한 OAuth 구현 방법을 배웁니다. 신규 애플리케이션은 여기서 시작하세요. [PKCE 가이드](/oauth/pkce)

#### HTTP API

REST API를 직접 호출하여 구현합니다. 가장 유연하지만 구현 복잡도가 높습니다. [HTTP API 기술 문서](/oauth/http), [인증 흐름 시작](/oauth/http/authorize), [토큰 교환](/oauth/http/token-exchange), [토큰 갱신](/oauth/http/token-refresh), [사용자 데이터 조회](/oauth/http/userinfo)

#### SDK

공식 SDK를 사용하면 OAuth 플로우를 간단하게 구현할 수 있습니다. [React SDK](/oauth/sdk/react), [Java/Kotlin SDK](/oauth/sdk/java)

#### 실전 예제

프레임워크별 완전한 구현 예제를 제공합니다. [React + Spring Boot (Kotlin)](/oauth/examples/react-spring-boot-kotlin)

### API 엔드포인트

```
https://oauth.authorization.datagsm.kr          # OAuth 인증 서버
https://oauth.resource.datagsm.kr # 사용자 데이터 서버
```
