...
REST API를 직접 호출하여 구현합니다. 가장 유연하지만 구현 복잡도가 높습니다. [HTTP API 문서](/oauth/http), [인증 흐름 시작](/oauth/http/authorize), [토큰 교환](/oauth/http/token-exchange), [토큰 갱신](/oauth/http/token-refresh), [사용자 정보 조회](/oauth/http/userinfo)
