...
PKCE (Proof Key for Code Exchange, RFC 7636)는 OAuth 2.0 Authorization Code Flow의 보안을 강화하는 확장 표준입니다. 원래는 모바일 앱과 같은 Public Client를 보호하기 위해 설계되었지만, 현재는 많은 OAuth 클라이언트에서 선택적으로 사용되고 있습니다.
...
생성한 `code_challenge`를 포함하여 `/v1/oauth/authorize`로 리다이렉트합니다.

```http
GET /v1/oauth/authorize?client_id=your-client-id&redirect_uri=https%3A%2F%2Fyour-app.com%2Fcallback&response_type=code&code_challenge=E9Melhoa2OwvFrEMTJguCHaoeK1t8URWbuGJSstw-cM&code_challenge_method=S256
```

서버는 사용자를 로그인 페이지로 리다이렉트하며, 로그인 성공 시 `redirect_uri`로 Authorization Code가 전달됩니다. 서버는 `code_challenge`를 Authorization Code와 함께 저장합니다.
...
async function initiateOAuthWithPKCE() {
  ...
  // 2. Authorization 요청 - /v1/oauth/authorize로 리다이렉트
  ...
  // 이후 사용자가 로그인하면 redirect_uri로 code가 전달됩니다.
  // redirect_uri 페이지에서 아래 3, 4단계를 처리하세요.
}

// redirect_uri 페이지에서 code를 받아 토큰 교환
async function handleCallback() {
  ...
  const tokenResponse = await fetch('https://oauth.authorization.datagsm.kr/v1/oauth/token', {
    method: 'POST',
...
export async function POST(request: NextRequest) {
  ...
  // 3. /v1/oauth/authorize로 리다이렉트 URL 생성 후 반환
  const params = new URLSearchParams({
  ...
  const authorizeUrl = `https://oauth.authorization.datagsm.kr/v1/oauth/authorize?${params}`;
...
export async function GET(request: NextRequest) {
  ...
  // 2. Token 교환
  const response = await fetch('https://oauth.authorization.datagsm.kr/v1/oauth/token', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      grant_type: 'authorization_code',
  ...
  <div className="flex items-start gap-3">
    ...
    <div>
      ...
      <div className="text-blue-800 dark:text-blue-200 text-sm">
        <code className="mx-1 px-1.5 py-0.5 bg-blue-100 dark:bg-blue-800 rounded">code_verifier</code>는 안전하게 저장되어야 합니다.
        브라우저 환경에서는 httpOnly 쿠키를 사용하시면 더 안전합니다.
...
|------|--------|----------|-----------|------|
| **httpOnly 쿠키** | ⭐⭐⭐⭐⭐ | ✅ | ✅ 추천 | JavaScript에서 접근 불가, 가장 안전 |
| **서버 세션** | ⭐⭐⭐⭐⭐ | ✅ | ✅ 추천 | 서버 메모리/DB에 저장, 매우 안전 |
| **sessionStorage** | ⭐⭐⭐ | ❌ | 💡 참고 | XSS 취약점 주의 필요 |
| **localStorage** | ⭐⭐ | ❌ | 💡 참고 | XSS 취약점이 있어 주의 필요 |
| **메모리** | ⭐⭐⭐⭐ | ✅ | ✅ 추천 | 새로고침 시 손실, SPA에 적합 |
...
자세한 구현 방법은 [React + Spring Boot 예제](/oauth/example/react-spring)를 참고하세요.
...
`plain` 방식은 `code_verifier`를 그대로 전송하므로, 중간자 공격에 취약합니다. OAuth 2.0 스펙에서도 `S256` 사용을 제안하고 있습니다.
...
- PKCE만 사용하셔도 충분한 보안을 제공합니다
- `client_secret`을 함께 사용하면 관리가 조금 복잡해질 수 있습니다
- 대부분의 OAuth 제공자는 PKCE를 사용할 때 `client_secret`을 선택 사항으로 만듭니다

**제안**: 꼭 필요하지 않다면 PKCE만 사용하시는 것이 관리가 편할 수 있습니다.
...
- [React + Spring Boot 예제](/oauth/example/react-spring) - BFF 패턴 기반 PKCE 구현
- [HTTP API 기술 문서](/oauth/http/authorize) - PKCE 파라미터 상세 설명
- [토큰 교환](/oauth/http/token-exchange) - code_verifier 사용 방법
