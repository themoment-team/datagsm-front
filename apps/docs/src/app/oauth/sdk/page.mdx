...
|-------------------|-----------|-----------------|-----------------------------------|---------------------------------------|
| **React (JS/TS)** | React 18+ | npm, yarn, pnpm | Hook 기반 로직, 로그인 버튼 컴포넌트 제공, TS 지원 | [React SDK 문서](/oauth/sdk/react) |
| **Java**          | JDK 13+   | Gradle, Maven   | 타입 안전성, 빌더 패턴, AutoCloseable      | [Java SDK 문서](/oauth/sdk/java)   |
| **Kotlin**        | 1.3.72+   | Gradle, Maven   | Java SDK와 완벽 호환, 확장 함수 지원         | [Java SDK 문서](/oauth/sdk/java)   |
...
1. **프론트엔드 (React)**: [React SDK 문서](/oauth/sdk/react)에서 설치 및 버튼 연동 방법을 확인하세요.
2. **백엔드 (Java/Kotlin)**: [Java SDK 문서](/oauth/sdk/java)에서 서버 측 토큰 처리 및 데이터 조회 방법을 확인하세요.

### 추가 리소스

- **OAuth 개념 이해**: SDK 사용 전 OAuth 플로우가 궁금하다면 [OAuth 개요](/oauth/)를 읽어보세요.
- **REST API 명세**: SDK 없이 직접 연동하려면 [OAuth HTTP API](/oauth/http) 문서를 참고하세요.
- **GitHub 저장소**:
