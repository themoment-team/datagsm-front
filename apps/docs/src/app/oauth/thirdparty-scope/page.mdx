...
서드파티 권한 범위를 관리하기 위한 REST API입니다. 모든 요청에는 인증이 필요하며, Application 소유자 또는 관리자 권한이 있어야 합니다.

#### 권한 범위 추가

```http
POST /v1/applications/{id}/scopes
```

Application에 새로운 권한 범위를 추가합니다.
...
| 파라미터          | 타입       | 필수 여부 | 설명                                                  | 예시                |
|---------------|----------|-------|-----------------------------------------------------|-------------------|
| `scopeName`   | `String` | 필수    | 권한 범위 이름 (소문자 영문, 숫자, `_`, `-`만 허용, 최대 100자)       | `profile`         |
| `description` | `String` | 필수    | 권한 범위 설명 (최대 255자)                                  | `사용자 프로필 정보 조회`   |

##### 요청 예시

```bash
curl -X POST "https://oauth.authorization.datagsm.kr/v1/applications/a1b2c3d4-e5f6-7890-abcd-ef1234567890/scopes" \
...
```http
PATCH /v1/applications/{id}/scopes/{scopeId}
```

기존 권한 범위의 이름이나 설명을 수정합니다.
...
|-----------|----------|----------------|
| `id`      | `String` | Application ID |
| `scopeId` | `Long`   | 권한 범위 ID       |

##### 요청 본문

| 파라미터          | 타입       | 필수 여부 | 설명                                                  | 예시                |
|---------------|----------|-------|-----------------------------------------------------|-------------------|
| `scopeName`   | `String` | 필수    | 변경할 권한 범위 이름 (소문자 영문, 숫자, `_`, `-`만 허용, 최대 100자)   | `user_profile`    |
| `description` | `String` | 필수    | 변경할 권한 범위 설명 (최대 255자)                              | `사용자 프로필 조회`   |

##### 요청 예시

```bash
curl -X PATCH "https://oauth.authorization.datagsm.kr/v1/applications/a1b2c3d4-e5f6-7890-abcd-ef1234567890/scopes/1" \
  -H "Content-Type: application/json" \
  -H "Cookie: SESSION=your-session-cookie" \
  -d '{
    "scopeName": "user_profile",
    "description": "사용자 프로필 상세 조회"
  }'
```
...
```http
DELETE /v1/applications/{id}/scopes/{scopeId}
```

Application에서 권한 범위를 삭제합니다.
...
```bash
curl -X DELETE "https://oauth.authorization.datagsm.kr/v1/applications/a1b2c3d4-e5f6-7890-abcd-ef1234567890/scopes/1" \
  -H "Cookie: SESSION=your-session-cookie"
```
...
```http
GET /v1/clients/scopes
```

OAuth Client에 등록할 수 있는 모든 사용 가능한 권한 범위 목록을 조회합니다.

##### 응답 형식

| 필드                | 타입       | 설명                                              | 예시                                             |
|-------------------|----------|-------------------------------------------------|------------------------------------------------|
| `scope`           | `String` | 권한 범위 문자열 (`applicationId:scopeName`)            | `a1b2c3d4-e5f6-7890-abcd-ef1234567890:profile` |
| `description`     | `String` | 권한 범위 설명                                        | `사용자 프로필 정보 조회`                                |
| `applicationName` | `String` | 권한 범위가 속한 Application 이름                        | `My Application`                               |
...
```json
{
  "list": [
    ...
    {
      "scope": "a1b2c3d4-e5f6-7890-abcd-ef1234567890:email",
